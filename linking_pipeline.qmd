...
1.  Joining `ESD` (which has one row per **beep**) with `all_usage` (which has many rows of phone usage events) **by user**. This results in a many-to-many relationship for the merge, which we need to specify as the `relationship` in `left_join()` to suppress the associated warning.

::: callout-note
## Note

This is arguably not very efficient, as we are not only joining a app usage rows that are outside of any beep time window, but we also join together app usage data with **every beep** of the current user. From a data perspective, that's not a big deal, since we will `filter()` out all app usages that are not within the correct time window in the next step 2. For this dataset, the runtime for this joining operation should still be \< 10 min on average hardware, which is really humane all things considered. The benefit of this lazy approach is that the code is much simpler and clearer, which is a win-win for us (the programmers) and people who would want to reproduce this in the future. You are welcome!(?). A drawback is that this step becomes quite memory-intensive, which we will need to address, see below.
:::
...
4.  Group by `userID`, `survey_number`, `beep_time`, and `better_category`
...
enrich_esd_with_psd_per_user <- function(ESD, PSD, app_categories, time_window){
  ...
  ESD |> 
    ...
    # 2) Keep only app usage events that occur within the `time_window` before `beep_time`
    filter(
      !is.na(beep_time),
      !is.na(start_time),
    ...
    left_join(
      ...
      # this is a more reasonable relationship that won't cause a warning anyway, but there is no harm in telling dplyr what we expect. Also, this way, it will tell us if something unexpeted is happening!
...
The summarization step gave us a dataset (`usage_summary`) with one row per **UserID, beep_time and category.** Next up, we want to merge these back into the original ESD so we get one row per beep, one columns for each app category’s usage time and open frequency. To get there we must first pivot wide:
...
# We create named rows fromt he names of the two vector names we bind
# which gives us two rows here of n columns where n is the number of
# app categories
# We then convert that dataframe back into a vector, which by default 
# collects elements by going line by line and then column by column.
# Not super intuitive, but it does exactly what we need here.
...
```{r}
# make sure we keep 
filename <- tools::file_path_sans_ext(basename(esm_file))
